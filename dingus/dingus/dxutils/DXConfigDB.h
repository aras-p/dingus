#ifndef __DX_CONFIGDATABASE_H__
#define __DX_CONFIGDATABASE_H__
...
public:
	...
	static IDXConfigDB* Create();
	
	/**
	 * Load the config information in the text file given.	See below for information
	 * on the format of the text file.	SystemMemory is in MB.
	 * CPU Speed is in MHz.
	 */
	virtual bool Load( const WCHAR* FileName, const CD3DDeviceCaps& caps, DWORD SystemMemory ) = 0;
...
//
// The program first searches for DisplayVendor = 0xXXXX that matches the current card.
...
//
// Other conditions include all the DWORD members of the D3DCAPS8 structure.  For example,
//
// DevCaps & Flag
//
